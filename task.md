...
## Current Work Order: WO-016-A — Codex Review Fix: PIT As-Of Ordering + Strict OHLCV Timestamp Validation

### 목적
Codex bot PR review 반영: PIT getAsOf 정렬 기준 수정, OHLCV timestamp strict ISO 검증 및 data.go.kr URL 등록 금지 정책 문서화.

### Checklist
- [x] Phase 1: PIT getAsOf 정렬 우선순위 수정 (asOfDate desc, ingestedAt desc, effectiveAt desc, pitRecordId desc)
- [x] Phase 2: OHLCV timestamp strict ISO 검증 구현 및 Date.parse 의존성 제거
- [x] Phase 3: data.go.kr API provider URL 등록 제한 정책 문서 반영
- [x] Phase 4: PR #2 Codex Review 내용 반영 문서화 및 task.md 갱신
