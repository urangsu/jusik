...
- **WO-017-C**: Individual Signal IC Audit
- **WO-017-D**: Factor/Strategy Correlation + Market Exposure
- **WO-017-E**: Signal Stability Gate
- **WO-017-F**: Structured Output Guard

---

## Current Work Order: WO017-B — Strategy Trial Memory + Signal Postmortem Skeleton

### 목적
백테스트 결과를 전략 연구 기록으로 저장하고, OOS window별 선택 자산 상세를 Signal Postmortem skeleton으로 변환하여 보존 및 반복 실험을 추적한다.

### Checklist
- [ ] Domain types 정의 (`StrategyTrialRecord`, `SignalPostmortem`, etc.)
- [ ] Strategy Trial ID 및 Parameter Hash 구현
- [ ] Strategy Trial Store 및 Signal Postmortem Store 구현
- [ ] BacktestResult → StrategyTrialRecord 변환기 구현
- [ ] Backtest selectedPositions → SignalPostmortem 변환기 구현
- [ ] CLI script (`strategy:trial:from-backtest`) 구현
- [ ] API routes (GET `/api/strategy/trials`, `/api/strategy/signal-postmortems`) 구현
- [ ] UI 최소 연결 (StrategyTrialMemoryPanel, StrategyTrialTable, SignalPostmortemSummary)
- [ ] Unit & integration tests 작성
- [ ] Docs 작성 (`docs/STRATEGY_TRIAL_MEMORY.md`, `docs/SIGNAL_POSTMORTEM_POLICY.md`)
- [ ] Verification (typecheck, lint, test, build, wording)
...
1. **WO017-B Strategy Trial Memory**: 전략 시도 기록 및 전략 묘지 지원.
2. **WO017-C Individual Signal IC Audit**: 개별 시그널별 IC 감사 리포트.
3. **WO017-D Factor/Strategy Correlation + Market Exposure**: 팩터/전략 상관관계 및 시장 익스포저 감사.
