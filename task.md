...
## Current Work Order: WO017-C — Individual Signal IC Audit + Strategy Trial Memory Linkage

### 목적
전략에 기여하는 개별 atomic signal들의 예측력을 OOS 기준 단면 Spearman IC, ICIR 및 Hit Rate로 개별 검증하고, 그 결과를 Strategy Trial Memory UI와 연동한다.

### Checklist
- [x] Domain types 정의 (`IndividualSignalIcResult`, assessments, warnings)
- [x] Signal Candidate Resolver 구현
- [x] Signal Score Series Loader 구현
- [x] Forward Return Series Loader 구현
- [x] Individual Signal IC Auditor 구현
- [x] Audit Result Store 구현 및 최신 스냅샷 저장
- [x] CLI script (`audit:individual-signal-ic`) 구현
- [x] API routes (`GET /api/audit/individual-signal-ic` & run route) 구현
- [x] UI 연동 (`IndividualSignalIcSummary` & `StrategyTrialMemoryPanel` 임베딩)
- [x] Unit & integration tests 작성
- [x] Docs 작성 (`docs/INDIVIDUAL_SIGNAL_IC_AUDIT.md`)
