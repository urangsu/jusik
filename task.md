...
- **WO-017-B**: Strategy Trial Memory
...
## Current Work Order: WO017-A — Backtest Validity Hardening for momentum_v1_long_only

### 목적
단일 전략 후보(`momentum_v1_long_only`)의 백테스트 결과를 기능 검증 수준으로 보강한다. selectedPositions 추적, IC score 연결, 복리 totalReturn, turnover, benchmark/excess return, validityReport를 추가한다.

### Checklist
- [x] selectedPositions / signalScore / rank / entry-exit 기록
- [x] IC pair에 실제 signalScore 연결 및 validIcPairCount
- [x] totalReturn 복리 누적
- [x] turnover 계산 (첫 window null, 이후 평균)
- [x] benchmarkReturn / excessReturn + missing_benchmark 경고
- [x] BacktestValidityReport (functional_check_only 기본)
- [x] CLI invalid strategy/universe 실패 처리
- [x] UI Validity Badge / Selected Positions / OOS table 보강
- [x] docs/BACKTEST_ENGINE.md, docs/BACKTEST_VALIDITY_POLICY.md
- [x] Verification (typecheck, lint, test, build, docs, wording)
