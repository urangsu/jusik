...
export async function POST(request: NextRequest) {
  try {
    ...
    const { findingId, locale, userPrompt, mode } = body;
    ...
    const validModes = ["safe", "forbidden_wording", "ungrounded_claim", "missing_disclaimer"];
    const chosenMode = validModes.includes(mode) ? mode : "safe";

    const result = await createGuardedMockAiOutput({
      findingId,
    ...
    const envelope = {
      value: result,
      status: result.output.isBlocked ? ("error" as const) : ("cached" as const),
      source: "guarded_ai_output_service",
