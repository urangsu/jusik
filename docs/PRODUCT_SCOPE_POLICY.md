...
8. **AI Provider Interface 및 Disabled-by-Default Registry**: AI Provider Interface는 실제 외부 AI 호출 기능이 아닙니다. 모든 외부 provider(OpenAI, Anthropic, Gemini, local)는 기본 disabled 상태이며, `not_supported`를 반환합니다. mock provider만 deterministic 검증용으로 활성화 상태입니다. provider output도 반드시 Structured Output Guard를 통과해야 합니다.
