...
1. **외부 알림 연동 불가**: 이메일, SMS, Telegram, KakaoTalk 등 외부 메신저 및 푸시 알림과의 연계를 금지하며 오직 사내용 대시보드(Web Inbox) 형태로만 정보를 제공합니다.
2. **리서치 자료 전문 보존 및 크롤링 금지**: 유료 증권사 리포트 자동 수집, PDF 전문 저장, 뉴스 전문 크롤링 저장을 일체 불허하며, 오직 원문 링크 제공으로 저작권을 보호합니다.
3. **AI 요약 배제**: 수집된 보고서 또는 이벤트 요약에 LLM의 비확정적 생성 요약(summarization) 기능을 쓰지 않습니다.
4. **투자 추천 문구 사용 금지**: UI 상에 매수/매도 확정, 수익 보장 등의 표현을 금지하며 오직 진단성 정보만 노출합니다.
