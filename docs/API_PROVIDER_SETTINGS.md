...
| `fred` | FRED | 미국 연준 거시 경제 데이터 수집 (추후 도입) | `FRED_API_KEY`, `FRED_ENABLED` |

> [!NOTE]
> `telegram`, `email`, `llm` 공급자 설정은 UI 설정 및 암호화 저장을 지원하나, 실제 외부 연동 기능은 [PRODUCT_SCOPE_POLICY.md](file:///Volumes/무제/jusik/docs/PRODUCT_SCOPE_POLICY.md)에 의거하여 현 상태로 동결(Frozen)되어 동작합니다. `fred` 연동은 본 제품 범위에서 제외되어 비활성 상태입니다.
...
## 4. 공급자 엔드포인트 등록 제약 정책 (Endpoint Registration Constraints)

- **공공데이터포털(data.go.kr) 관련 정책**:
  - `https://www.data.go.kr/iim/api/selectAcountList.do`와 같은 공공데이터포털 로그인/활용계정 관리 화면 URL은 데이터 수집용 API endpoint가 아닙니다.
  - 공공데이터포털 관리 화면 URL은 provider endpoint로 절대 등록하지 않으며, 오직 개별 데이터셋의 실제 OpenAPI endpoint만 provider 후보가 될 수 있습니다.
